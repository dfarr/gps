/*****************
shared.c -- utilities for a gps file parser
...
Also include Python "wrapper" functions,
makefile sets up the shared library Gps.so
...
/* static pointer needed for interaction with python */
static GpFile *filep;
static GpTrack *track;
...
/* Python Wrapper function for readGpFile and getGpTracks */
PyObject *Gps_readFile(PyObject *self, PyObject *args)
{
	GpStatus status;
	filep = malloc(sizeof(GpFile));
	track = malloc(sizeof(GpTrack));
	...
	{
		if(errno == EACCES)
			return Py_BuildValue("s", "Failed to open the specified file: Permission Denied");
		else if(errno == EBADF)
			return Py_BuildValue("s", "Failed to open the specified file: Bad File Descriptor");
		else if(errno == EBADFD)
			return Py_BuildValue("s", "Failed to open the specified file: File descriptor in bad state");
		else if(errno == EFBIG)
			return Py_BuildValue("s", "Failed to open the specified file: File too large");
		else if(errno == EIO)
			return Py_BuildValue("s", "Failed to open the specified file: Input/Output Error");
		else if(errno == EISDIR)
			return Py_BuildValue("s", "Failed to open the specified file: Is a directory");
		else if(errno == EMFILE)
			return Py_BuildValue("s", "Failed to open the specified file: Too many open files");
		else if(errno == ENFILE)
			return Py_BuildValue("s", "Failed to open the specified file: Too many open files in system");
		else if(errno == ENOENT)
			return Py_BuildValue("s", "Failed to open the specified file: No such file or directory");
		else if(errno == ESTALE)
			return Py_BuildValue("s", "Failed to open the specified file: Stale file handle");
		else if(errno == ETXTBSY)
			return Py_BuildValue("s", "Failed to open the specified file: File is busy");
		else
			return Py_BuildValue("s", "Failed to open the specified file: Uknown File");
		
	}
	
	status = readGpFile(input, filep);
	fclose(input);
	...
	else
	{
		...
		if(status.code == IOERR)
			return Py_BuildValue("si", "An I/O Error occured on line", status.lineno);	
		else if(status.code == UNKREC)
			return Py_BuildValue("si", "An Unknown Record Error occured on line", status.lineno);	
		else if(status.code == BADSEP)
			return Py_BuildValue("si", "A Bad Seperation Error occured on line", status.lineno);	
		else if(status.code == FILTYP)
			return Py_BuildValue("si", "A File Type Error occured on line", status.lineno);	
		else if(status.code == DATUM)
			return Py_BuildValue("si", "A Datum Error occured on line", status.lineno);		
		else if(status.code == COORD)
			return Py_BuildValue("si", "A Coordinate Error occured on line", status.lineno);	
		else if(status.code == NOFORM)
			return Py_BuildValue("si", "A No Form Error occured on line", status.lineno);	
		else if(status.code == FIELD)
			return Py_BuildValue("si", "A No Field Error on line", status.lineno);
		else if(status.code == VALUE)
			return Py_BuildValue("si", "A Value Error occured on line", status.lineno);
		else if(status.code == DUPRT)
			return Py_BuildValue("si", "A Duplicate Route Error occured on line", status.lineno);
		else if(status.code == UNKWPT)
			return Py_BuildValue("si", "An Unknown Waypoint Error occured on line", status.lineno);
		else
			return Py_BuildValue("i", "An Unknown Error has occured");
	}
	
	return NULL;
}

/* Python wrapper function for filling four lists with:
   waypoints, routes, trackpoints and tracks */
PyObject *Gps_getData(PyObject *self, PyObject *args)
{
	PyObject *waypts, *routes, *trkpts, *tracks;
	int check = 0;
	
	/* convert the objects to c pointers */
	if(!PyArg_ParseTuple(args, "OOOO", &waypts, &routes, &trkpts, &tracks))
		return NULL;
	
	/* make space in the lists */
	waypts = PyList_New(filep->nwaypts);
	routes = PyList_New(filep->nroutes);
	trkpts = PyList_New(filep->ntrkpts);
	tracks = PyList_New(numTracks);
	
	/* convert waypoints, routes and trackpoints */
	for(int index = 0; index < filep->nwaypts; index++)
	{
		check = PyList_SetItem(waypts, index, Py_BuildValue("ffss", filep->waypt[index].coord.lat, filep->waypt[index].coord.lon, filep->waypt[index].ID, filep->waypt[index].comment));
		if(check == -1)
			return NULL;
	}
	...
	for(int index = 0; index < filep->nroutes; index++)
	{
		...
		/* make a list for the legs */
		legs = PyList_New(filep->route[index]->npoints);
		for(int k = 0; k < filep->route[index]->npoints; k++)
			PyList_SetItem(legs, k, Py_BuildValue("i", filep->route[index]->leg[k]));
		
		check = PyList_SetItem(routes, index, Py_BuildValue("isO", filep->route[index]->number, filep->route[index]->comment, legs));
		if(check == -1)
			return NULL;
	}
	...
	{
		check = PyList_SetItem(trkpts, index, Py_BuildValue("ff", filep->trkpt[index].coord.lat, filep->trkpt[index].coord.lon));
		if(check == -1)
			return NULL;
	}
	...
	{
		char time[30], gTime[30];
		strcpy(time, ctime(&track[index].startTrk));
		...
		check = PyList_SetItem(tracks, index, Py_BuildValue("is", track[index].seqno, gTime));
		if(check == -1)
			return NULL;
	...
	return Py_BuildValue("OOOO", waypts, routes, trkpts, tracks);
}

/* Python wrapper functions for freeing the structures */
PyObject *Gps_freeFile(PyObject *self, PyObject *args)
{
	/* free the GpFile structure */
	freeGpFile(filep);
	free(filep);
	filep = NULL;
	
	/* free the GpTrack structure */
	if(numTracks > 0)
		free(track);
	track = NULL;
